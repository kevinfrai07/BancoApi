using Banco.Domain.DTO;
using Banco.Domain.Entity;
using Banco.Persistence;
using MediatR;
using Microsoft.EntityFrameworkCore;


namespace Banco.Service.Features.CuentaFeatures.Queries
{
    public class GetCuenta : IRequest<ResponseDTO>
    {
        public class GetUsersHandler : IRequestHandler<GetCuenta, ResponseDTO>
        {
            private readonly IApplicationDbContext _context;

            public GetUsersHandler(IApplicationDbContext context)
            {
                _context = context;
            }

            public async Task<ResponseDTO> Handle(GetCuenta request, CancellationToken cancellationToken)
            {
                ResponseDTO respuesta = new ResponseDTO();
                string error = "Error consultando tabla Cuenta";

                try
                {
                    var cuentas = await _context.Cuentas
                                .Join(_context.Clientes,
                                    cu => cu.ClienteId,
                                    cl => cl.ClienteId,
                                    (cu, cl) => new { cu, cl })
                                .Join(_context.Personas,
                                    temp => temp.cl.PersonaId,
                                    pe => pe.PersonaId,
                                    (temp, pe) => new CuentaDTO
                                    {
                                        NumeroCuenta = temp.cu.NumeroCuenta,
                                        TipoCuenta = temp.cu.TipoCuenta,
                                        SaldoInicial = temp.cu.SaldoInicial,
                                        Estado = temp.cu.Estado,
                                        Cliente = pe.Nombre
                                    })
                                .ToListAsync();
                    object responseData = new object();

                    if (cuentas.Count == 0)
                    {
                        error = "Cuentas no encontrados";

                        respuesta.responseStatus = 200;
                        respuesta.responseData = new
                        {
                            Cuenta = "",
                        };

                        return respuesta;
                    }


                    respuesta.responseStatus = 200;
                    respuesta.responseData = cuentas;
                    return respuesta;
                }
                catch (Exception e)
                {
                    respuesta.responseStatus = 500;
                    respuesta.responseData = new
                    {
                        error = error
                    };
                    return respuesta;
                }
            }
        }
    }
}
